...
def medio_sudoku_filter(lista_0, lista_1):
    lista_01 = []
    cont = 0
    for lista0 in lista_0:
        for lista1 in lista_1:
            medio_sudoku = np.concatenate([lista0,lista1],axis=0)
            # Check
            c = 0
            for row in range(medio_sudoku.shape[0]):
                if len(np.unique(medio_sudoku[row,:])) < medio_sudoku.shape[1]:
                    break
                else:
                    c += 1
            if c == medio_sudoku.shape[0]:
                for col in range(medio_sudoku.shape[1]):
                    if len(np.unique(medio_sudoku[:,col])) < medio_sudoku.shape[0]:
                        break
                    else:
                        c += 1
            if c == sum(medio_sudoku.shape):
                #print medio_sudoku
                lista_01.append(medio_sudoku)
        cont += 1
        #print float(cont)/(float(len(lista_0))+float(len(lista_1)))
    return lista_01
...
def sudokuto(info):
    ...
    lista_01 = medio_sudoku_filter(lista_0,lista_1)
    ...
    cont = 0
    for lista01 in lista_01:
        for lista2 in lista_2:
            sudoku = np.concatenate([lista01,lista2],axis=0)
            # Check
            c = 0
        ...
        cont += 1
        #print float(cont)/(float(len(lista_01))+float(len(lista_2)))
